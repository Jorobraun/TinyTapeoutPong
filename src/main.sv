...
module Pong #(
  ...
  Schlaeger p1_ctrl (.clk(clk), .reset(reset), .frame_tick(frame_tick), .move_down(player1_down), .move_up(player1_up), .pos(player1_y));
  Schlaeger p2_ctrl (.clk(clk), .reset(reset), .frame_tick(frame_tick), .move_down(player2_down), .move_up(player2_up), .pos(player2_y));
  
  Schlaeger_collide phys_p1 (.pos(player1_y), .mesure_pos(ball_y_pos), .res(ball_hits_p1_y));
  Schlaeger_collide phys_p2 (.pos(player2_y), .mesure_pos(ball_y_pos), .res(ball_hits_p2_y));
  ...
  always @(posedge clk) begin
    if (reset) begin
      ball_x_pos <= 48; ball_y_pos <= 32;
      ball_x_dir <= 0;  ball_y_dir <= 0;
      scoreP1 <= 8'h00; scoreP2 <= 8'h00;
      ...
      if (ball_x_pos >= 98 || ball_x_pos <= 1) begin
        ...
        if (ball_y_pos == 0) begin ball_y_dir <= 0; ball_y_pos <= 1; end
        else if (ball_y_pos == 63) begin ball_y_dir <= 1; ball_y_pos <= 62; end
        else ball_y_pos <= ball_y_dir ? (ball_y_pos - 1'b1) : (ball_y_pos + 1'b1);

        if (ball_x_dir == 1 && ball_x_pos == player1_schlaeger_x + 1'b1 && ball_hits_p1_y) begin
          ball_x_dir <= 0; ball_x_pos <= player1_schlaeger_x + 2;
        end else if (ball_x_dir == 0 && ball_x_pos == player2_schlaeger_x - 1'b1 && ball_hits_p2_y) begin
          ball_x_dir <= 1; ball_x_pos <= player2_schlaeger_x - 2;
  ...
  // Player 1 (Red) 
  Digit_Renderer p1t (.value(scoreP1[7:4]), .x_offset(7'd22), .y_offset(6'd5), .xpos(xpos), .ypos(ypos), .draw(d1t));
  Digit_Renderer p1o (.value(scoreP1[3:0]), .x_offset(7'd30), .y_offset(6'd5), .xpos(xpos), .ypos(ypos), .draw(d1o));
  // Player 2 (Green) 
  Digit_Renderer p2t (.value(scoreP2[7:4]), .x_offset(7'd60), .y_offset(6'd5), .xpos(xpos), .ypos(ypos), .draw(d2t));
  Digit_Renderer p2o (.value(scoreP2[3:0]), .x_offset(7'd68), .y_offset(6'd5), .xpos(xpos), .ypos(ypos), .draw(d2o));

  // Game Rendering
  wire coll_ball = (xpos == ball_x_pos) && (ypos == ball_y_pos);
  wire draw_p1, draw_p2;
  Schlaeger_collide disp_p1 (.pos(player1_y), .mesure_pos(ypos), .res(draw_p1));
  Schlaeger_collide disp_p2 (.pos(player2_y), .mesure_pos(ypos), .res(draw_p2));
...
module main (
  ...
  wire [6:0] hpos; wire [5:0] vpos;
  wire [1:0] r, g, b;

  hvsync_generator hvsync_gen (.clk(clk), .reset(~rst_n), .hsync(hsync), .vsync(vsync), .display_on(display_on), .hpos(hpos), .vpos(vpos), .frame_tick(frame_tick));

  Pong pong (
    .clk(clk), .frame_tick(frame_tick), .reset(~rst_n),
    .player1_down(ui_in[0]), .player1_up(ui_in[1]),
    .player2_down(ui_in[2]), .player2_up(ui_in[3]),
    .xpos(hpos), .ypos(vpos),
    .red(r), .green(g), .blue(b)
  );

  assign uo_out[0] = r[1]; assign uo_out[1] = g[1]; assign uo_out[2] = b[1];
  assign uo_out[3] = hsync;
  assign uo_out[4] = r[0]; assign uo_out[5] = g[0]; assign uo_out[6] = b[0];
